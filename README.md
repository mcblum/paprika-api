...
2. Each item is compared against what the connector already has, using a SHA-256 content hash.
3. New items are created, changed items are updated, unchanged items are skipped.
4. **Bidirectional:** items marked _Done_ in Notion are marked as purchased in Paprika on the same cycle.
...
docker run --env-file .env paprika-api
```
